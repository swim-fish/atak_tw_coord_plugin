 ...
 * facade to test whether a coordinate falls inside a 鄉鎮市區 polygon. The algorithm is the one proven
 * 8/8 against the real {@code townships.sqlite} by {@code scripts/verify_polygon_in.py}. Inputs
 * (the {@code geometry_wkb} blob) are treated as untrusted per Constitution VI: malformed bytes
 * recover to {@code null}, never throw.
 */
