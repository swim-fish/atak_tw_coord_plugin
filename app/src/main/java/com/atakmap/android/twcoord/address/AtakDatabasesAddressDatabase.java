 ...
 * Samsung One UI builds (verified on an Android 14 reference device, 2026-05-26): the runtime query
 * {@code SELECT … FROM places_rtree …} fails with {@code no such module: rtree}. ATAK's native
 * SQLite ({@link Databases#openDatabase}) bundles a full-featured build with R*Tree because ATAK
 * itself relies on R*Tree for its own spatial-index queries, so we reuse that runtime here. {@link
 * SqliteAddressDatabase} stays alive only for JVM/Robolectric unit tests which run against {@code
 * xerial sqlite-jdbc} (also R*Tree-enabled).
 */
public final class AtakDatabasesAddressDatabase implements AddressDatabaseFacade {
  ...
  public java.util.List<com.atakmap.android.twcoord.address.lookup.AddressCandidate>
    ...
    try {
      ...
      java.util.List<com.atakmap.android.twcoord.address.lookup.StreetCandidateRanker.Raw> raw =
          new java.util.ArrayList<>();
      try (CursorIface cursor = db.query(query.sql(), query.args())) {
