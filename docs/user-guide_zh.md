...
3. **完整地址**使用單一輸入框；切換成**結構化**後，先點選縣市，再點選
   鄉鎮市區；道路／地名與其餘地址仍可直接編輯。
...
縣市清單只會顯示已匯入的資料集；鄉鎮市區清單也只會顯示所選縣市資料中
實際存在的門牌行政區，不會因郵遞區號參考資料存在就宣稱可搜尋。若地圖
中心可由已安裝的行政區邊界嚴格判定，且該縣市／行政區可搜尋，該項目會
排在第一筆；其餘項目依中華郵政順序排列。未出現在郵遞參考資料中的匯入
項目仍會保留在清單末端。切換縣市會清除不相容的行政區，但會保留道路與
其餘地址，方便修正。
