...
**對應功能：** 006 縣市優先前向搜尋 · 008 介面改版 ｜ **對應版本：** v1.1.0 → **v1.3.0** ｜ **適用：** ATAK-CIV 5.5.0 – 5.7.x ｜ **語言：** 中文 · [English](tw-addr-search.md)
...
<p align="center">
  <img src="images/19-tw-addr-search-county-popup.jpg" alt="縣市選擇跳窗（依匯入的 townships.sqlite 決定數量與排序）" width="560"><br>
  <sub>按「清單…」跳出的<b>縣市選單</b>：依地理序排列（宜蘭 → 北部 → 西岸往南 → 台東/花蓮 → 離島），<b>目前縣市</b>以藍框高亮（此圖為台中市），<b>無地址資料</b>的縣市標 <b>⚠</b> 並變淡。<br>※ 清單只列出**已匯入的 <code>townships.sqlite</code>** 內含的縣市——此圖為中部資料包，故為 12 個；匯入全臺版邊界則會顯示全部 22 個。</sub>
</p>
