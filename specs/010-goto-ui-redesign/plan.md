...
| VI. Host-Process Isolation (NON-NEGOTIABLE) | The merged `goto_autofill` onClick routes through existing `safeClick(tag, runnable)` (catches `Throwable`). `styleMarkerModeRadio` **removes** `setBackgroundColor` in favour of a state-list drawable — eliminating, not adding, the `android.R.attr.*` misuse class that caused the 2026-05-16 crash. New drawables are concrete resource ids (no attr-ids). `findViewById` results null-checked at bind. Re-entrant submit guarded by existing `submitInFlight` AtomicBoolean. | PASS — change reduces VI risk surface |
