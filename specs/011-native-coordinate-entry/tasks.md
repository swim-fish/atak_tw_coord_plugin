...
- [X] T012 [US1] Create `app/src/main/res/layout/taiwan_coordinate_entry_pane.xml` with exactly one `native_entry_root` `ScrollView` and one `native_entry_content` vertical child using a 12 dp content inset; add 52 dp-high `native_entry_system_group` controls `native_entry_system_{taipower,twd97,twd67}`, `native_entry_pane_taipower`, a labelled 20 sp `native_entry_input_taipower` with 14 dp vertical padding, and `native_entry_status`, matching the equivalent dimensions in `app/src/main/res/layout/tw_coord_goto.xml` and adding no plugin-owned Confirm/Auto Fill/Clear/Copy/elevation/marker controls; then implement `app/src/main/java/com/atakmap/android/twcoord/nativeentry/TaiwanCoordinateEntryPane.java` against the public ATAK interface with UID `com.atakmap.android.twcoord.coordinateentry.taiwan`, plugin/localised resources, horizontal `GeoPointMetaData`, checked invalid errors, listener isolation, fatal-condition rethrow, and safe idempotent disposal until T007 is Green (FR-001/FR-002, FR-009, FR-016, FR-018, FR-021/FR-022, FR-027).
